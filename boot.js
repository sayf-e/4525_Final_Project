...
class BattackState {
    ...
    execute(me) {
      ...
      if(dist(me.position.x, me.position.y, mySorsar.position.x,  me.position.y) < 80)
        { 
          if(me.position.y <  mySorsar.position.y)
          {
            me.position.y += 3
          }
          if ( abs(mySorsar.position.y - me.position.y) < 10) 
          {
            me.changeState(0);
          }
        }
  ...
  class BchaseState {
    ...
    execute(me) {
      if(dist(me.position.x, me.position.y, mySorsar.position.x,  me.position.y) < 80)
      {
          ...
          if (mySorsar.position.x == me.position.x) {
            me.changeState(1);
        }
      }
  ...
  class boot { //enemy NPC
    constructor(x, y, o) {
  
      this.state = [new BchaseState(), new BattackState()];
      this.curState = 0;
    ...
    draw() 
    {
      image(bootImg, this.position.x, this.position.y)
