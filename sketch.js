...
class gameObj { //game object
  ...
  initialize() {
        ...
        for (var j = 0; j < this.tilemap[i].length; j++) {
            switch (this.tilemap[i][j]) {
                ...
                case 'e':  shoes.push(new shoe(j*20, i*20));
                   break;
                case 'b':  shoes.push(new boot(j*20, i*20));
                   break;
            }
        }
    }
    ...
    else if((game.gameState == 2) || (game.gameState == 5) ) //focus camera on player during gameplay
    {
      if((mySorsar.position.x > 200) && (mySorsar.position.x < 600))
      {
        this.xCor = 200- mySorsar.position.x;
      }
...
function mouseClicked() {
  ...
  else if(game.gameState == 3 || game.gameState == 4 || game.gameState == 6 || game.gameState == 7) //endscreen
  {
    if((mouseX > 160) && (mouseX < 240) && (mouseY > 240) && (mouseY < 280))
    {
    ...
    else if((mouseX > 160) && (mouseX < 240) && (mouseY > 285) && (mouseY < 330) && (game.gameState == 4))
    {
      mySorsar.position.set(200, 200)
      game = new gameObj;
      shoes = [];
...
function preload(){ //load images for sprites and environment
  ...
  bootImg = loadImage("images/boot.png")
  ...
  roachImg = loadImage("images/roach.png")
}
...
function draw() {
  ...
  else if(game.gameState == 6 || game.gameState == 7) //endscreen
  {
    c1 = color(0, 24, 181);
    c2 = color(0);
    for(let y=0; y<height; y++){
    ...
    {
      text('You Have Escaped Being Crushed!', 50, 50)
    }
    text("Return",170, 265);
    noFill();
    rect(160, 240, 80, 40);
